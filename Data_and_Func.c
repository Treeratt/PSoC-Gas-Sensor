...
uint16  WaitText(char *target, int time) 	/******************************************************/			 
 {
    ...
    { 							// читаем UART  
        ...
        }	// return true если все элементы строки найдены    if all chars in the target match
          else  index = 0;   //..........................     				// не тот фрагмент - обнуляемся 
    	  if( !UART_GetRxBufferSize())
            {
             CyDelay(1); 
             time--;
            }	// при проверке с==0 - вычитывает несколько раз, поэтому ReadRxData и GetRxBufferSize
    } //while(time);
...
void SetServer(int port)
{
    ...
    Test_Write(WaitText("OK", 500));  
    Test_Write(0);				// ждем ответа модуля
...
void http(char *output) 		 
{
     ...
     uint16 len=0;
     len=strlen(output)	;	
     //sprintf(sendString,"AT+CIPSEND\r",len);	
...
do{ 	// в буфере остался IP адрес
	  	...
	  	buff[count]= ch;	
        count++;	// добавляем в буфер
	}while(ch);
//LCD_Char_Position(1 ,0  ); 
    UART_UartPutString("IP:");
    UART_UartPutString(buff); 	// выведем IP на дисплей
}
